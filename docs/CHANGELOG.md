...
- **update-003**: Codexレビューの Herdr 対話方式への移行（開発ドキュメントテンプレート改訂の取り込み）
  - 反映元: `/home/sakagawa/git/DEV_TEMPLATE/template/` コミット `19e4977`（テンプレート側 update-004〜007 相当。前回取り込み `715db11` 以降の差分）
  - `CLAUDE.md`: 「Codexによるレビューの実行方法」を Herdr 対話方式に全面差し替え（レビューストリーム `rev-lift2d-{案件ID}`・エージェントのライフサイクル・依頼の送り方・`/new` による全文ゲート・結果保存〔full.log 廃止、トークン実測記録〕・並列時のサブエージェント委任）。「行き詰まり検出（全作業共通・必須）」節を新設。関連参照（feat/bug/update フロー・実験プロトコル・実装委任・Bash ルール・案件ディレクトリ構成・ディレクトリツリー）を整合
  - `AGENTS.md` 新規: Codex CLI が起動時に読むレビュー定型指示（「[AGENTS.md適用]」マーカー含む）。`docs/HERDR_SETUP.md` 新規: Herdr セットアップ手順書（テンプレートからコミット固定コピー、sha256 検証済み）
  - `docs/BACKLOG.md`: ステータス凡例（Open / In Progress / Review / On Hold / Closed / Cancelled）を追加、feat-013/014 の「中止」を Cancelled に表記統一、表の分断（空行）を修正。`.gitignore`: full.log ルールを削除（`*.log` 包括ルールで既存ファイルの ignore は維持）
  - レビューは本案件から新方式で実施（ブートストラップ。反復 codex-01〜03 + 全文ゲート codex-04 で高・中ゼロ収束、人レビュー承認済み）。コード変更なしのためテストは不要
