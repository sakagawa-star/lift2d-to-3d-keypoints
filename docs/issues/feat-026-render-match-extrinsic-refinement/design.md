...
[phase4 環境 (Python 3.10)]                     [matcher_lab 環境 (Python 3.12)]
 refine_extrinsics.py                            loftr_cli.py
  ...
  ├ Stage 1: 手動点PnP 初期値                        ├ 1280x720 縮小 + 逆写像
...
| 手動点PnP（入力全点、N≥6） | solvePnPRansac(iterationsCount=1000, reprojectionError=8.0, SOLVEPNP_ITERATIVE) → inlier で solvePnP(ITERATIVE, useExtrinsicGuess=True) | M4 §5-1（6点で実証）。多点初期値は M3/M3b の基準ポーズ経路で実証 |
...
| 受理（二峰仲裁） | 第1+第2 合計占有率 ≥ 0.9 かつ入力全手動点の再投影中央値の峰間差 ≥ 1.0px | M4 criteria §6.2（閾値）。D3 が多点手動点の仲裁を実証（固定判定は inlier 名簿中央値、全点中央値でも同一の採用結果。全点採用は入力だけで閉じるため） |
...
| 手動点PnP | seed 設定なし（プロセス開始直後の cv2 既定 RNG 状態で実行。M4/M4-2 と同一条件。少点数の RANSAC は inlier 集合が安定なため結果は決定的——M4-2 フェーズ0（6点）で再現一致を実証） |
...
- **処理ロジック**:
  ...
  3. 2D-3D 対応点を ObjectName で突合し、**一意性を明示検証**する（codex-66 指摘対応）:
     ...
     - 突合結果（交差集合）が**一意な ObjectName 6個以上**でなければエラー（点数と名前を列挙）。6個以上ある場合は**全点を使用**する（部分選択はしない）
     - 全座標が有限値（NaN/inf 拒否）、2D 座標が画像範囲 [0, W-1]×[0, H-1] 内、必須列（ObjectName, X, Y, Z / ObjectName, camera_name, X, Y）の欠損なし
- **エラーハンドリング**: 各失敗は「カメラ名: 理由（実測値と期待値）」の形式でレポートに記録。ファイル欠落・PLY 読込失敗・CSV の重複/欠損列/非有限値は全体エラーで exit 1、カメラ単位の不整合（画像サイズ・点数・2D 座標の画像範囲外）は当該カメラのみ失敗扱いで続行
- **境界条件**: 「重複1件+欠損1件で点数が6のまま」のケースは上記の一意 ObjectName 検証で検出される（点数カウントだけでは通ってしまうため一意性検証を必須とする）

### 4.2 手動点PnP 初期値（FR-002）

- **処理ロジック**: 入力された全手動点（N≥6）で §3.1 の「手動点PnP」行のとおり2段で解く。inlier < 4 または RANSAC 失敗で当該カメラ失敗（失敗段「手動点PnP」）
...
- **処理ロジック**（参照実装: `m3_1_sampling.py` の `cluster_analysis`。アルゴリズムを以下に固定）:
  ...
  6. 受理判定は §3.1 の「受理（単峰）」「受理（二峰仲裁）」のとおり。二峰仲裁は第1・第2クラスタそれぞれの中心解で入力全手動点の再投影誤差中央値を計算し、差 ≥ 1.0px なら誤差の小さい峰を採用
...
- 入力全手動点の再投影中央値（初期解・最終解）、警告閾値の点灯一覧をレポートに記録（合否に使わない）
...
- **診断レポート**（テキスト。`--out-report` パス）: 実行条件（入力パス・定数の要約・seed_base）、カメラごとの節（手動点PnP 結果（点数・inlier/outlier 名）、チェーンごとの反復ログ行、f_c/f2、受理形態、前提条件、警告一覧、手動点再投影）、全体サマリ（受理（単峰）/受理（仲裁）/不受理/失敗の内訳）
...
| 入力点数 | 一意6点以上・全点使用（ステップ7 差し戻しイテレーション1） | 当初の「ちょうど6点」は檻運用からの一般化で、病院の既存多点 CSV（17〜36点）を無加工で使えないヒアリング漏れだった。多点初期値（M3/M3b 基準ポーズ経路）と多点名簿の仲裁（D3）は実証済み。却下案: 自動6点選抜（選抜規則が新たな未検証要素になる）、手動6点サブセット作成（手間+偏った選択が退化を誘発） |
...
- 2026-08-02: **ステップ7 差し戻し（イテレーション1、investigation.md）対応**: 入力点数を「一意6点以上・全点使用」に変更（§3.1 表・§3.2・§4.1・§4.2・§4.5・§4.6・§7・§9。用語 6点PnP→手動点PnP、仲裁・記録の物差しは入力全手動点の再投影中央値。RANSAC パラメータ・ゲート・サンプリング・クラスタ分析は不変）
