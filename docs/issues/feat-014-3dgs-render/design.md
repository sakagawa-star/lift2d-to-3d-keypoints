# 機能設計書: feat-014 ピンホール3DGSレンダリング（PNG出力）

## 1. 対象ファイル

`phase4/render_keypoints.py` を**3DGSレンダリングのみに作り直す**（feat-013 で書いた機能を削除し縮小）。
...
**テスト**: `tests/test_feat014_render.py` を新規作成（純粋ロジック）。既存 `tests/test_feat013_render_keypoints.py` は削除した関数（`distortion_to_gsplat`, `extract_halpe26` 等）を import しており残すと collect 失敗するため、**削除する**（キーポイント・オクルージョンのテストは feat-013 再開時に新規作成）。テスト結果は `tests/results/feat-014_test_result.txt`。

**依存**: `opencv-python`, `tomli` を使用。`c3d` は本ステップ未使用だが pyproject からは外さない（feat-013 で使う）。`render.py` の `load_ply`, `print_ply_summary` を再利用。
...
- `D`（distortions）は読むがレンダリングでは未使用（ピンホール）。feat-013 で使うため保持
- bpy/CUDA非依存 → テスト対象
...
- 実コード相当の snippet（**`render.py` の `render_frame` と同一の古典経路**。歪み・UT・深度なし）:
  ...
  # camera_model="pinhole"・with_ut=False・packed=True はいずれもデフォルト = render.py render_frame と同一
...
### 3.5 `main()`
- 引数解析 → `from render import load_ply, print_ply_summary`（関数内）→ `load_cameras_toml` → `select_camera`（不在は stderr＋exit1）→ `load_ply`＋`print_ply_summary` → `render_image` → `cv2.imwrite(output, bgr)`
...
| PLY/TOMLファイル不在 | `load_ply`/`tomli.load`/`cv2.imwrite` の標準例外に委譲（トレースバック表示。スコープ最小化のため独自処理しない） |

## 7. テスト設計

### 7.1 ユニット（純粋ロジック、bpy/CUDA非依存）— `tests/test_feat014_render.py`
...
- ディレクトリ構成: `render_keypoints.py` の役割（ピンホール3DGSレンダリング。feat-013 でキーポイント重ね描き・オクルージョンを追加予定）
