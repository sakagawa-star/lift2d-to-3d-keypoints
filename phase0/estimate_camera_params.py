...
使い方:
    python estimate_camera_params.py config.yaml                    # 通常
...
def run_estimation(config_path: str, use_k3: bool, fix_center: bool):
    ...
    # 必要点数の決定
    if fix_center:
        min_points = MIN_POINTS_DIST5_FIXCENTER if use_k3 else MIN_POINTS_DIST4_FIXCENTER
    else:
        min_points = MIN_POINTS_DIST5 if use_k3 else MIN_POINTS_DIST4
...
def main():
    parser = argparse.ArgumentParser(
        description='カメラ パラメータ推定（主点固定オプション対応版）',
...
使用例:
    ...
    run_estimation(args.config, args.k3, args.fix_center)
    return 0
